...
import { useShare, canUseWebShare, canUseClipboard, getShareCapabilities } from '../useShare';
...
describe('useShare', () => {
  ...
  describe('shareContent function', () => {
    ...
    test('should fallback to clipboard when Web Share fails', async () => {
      ...
      expect(mockNavigator.clipboard.writeText).toHaveBeenCalledWith('https://example.com');
    ...
    test('should use manual method when all else fails', async () => {
      ...
      expect(prompt).toHaveBeenCalledWith('请手动复制以下链接:', 'https://example.com');
    });
