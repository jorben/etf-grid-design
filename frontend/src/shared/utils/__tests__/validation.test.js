...
describe('validation utils', () => {
  ...
  describe('validatePercentage', () => {
    ...
    test('should validate with custom range', () => {
      expect(validatePercentage(0.1, { min: 0.1, max: 0.9 })).toEqual({ isValid: true, error: '' });
  ...
  describe('validateDate', () => {
    ...
    test('should validate YYYYMMDD format', () => {
      expect(validateDate('20240315', 'YYYYMMDD')).toEqual({ isValid: true, error: '' });
      expect(validateDate('20241225', 'YYYYMMDD')).toEqual({ isValid: true, error: '' });
    });
  ...
  describe('validateRequired', () => {
    test('should validate required fields', () => {
      expect(validateRequired('value', '字段名')).toEqual({ isValid: true, error: '' });
      expect(validateRequired(0, '字段名')).toEqual({ isValid: true, error: '' });
      expect(validateRequired(false, '字段名')).toEqual({ isValid: true, error: '' });
    });
  ...
  describe('validateNumberRange', () => {
    test('should validate numbers within range', () => {
      expect(validateNumberRange(50, { min: 0, max: 100 }, '年龄')).toEqual({ isValid: true, error: '' });
    });
    ...
    test('should reject invalid input', () => {
      expect(validateNumberRange('invalid', { min: 0, max: 100 }, '年龄')).toEqual({ 
        isValid: false, 
        error: '年龄必须是有效的数字' 
      });
    });
