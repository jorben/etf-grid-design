...
class ETFService:
    ...
    def get_etf_basic_info(self, etf_code: str) -> Optional[ETFBasicInfo]:
        ...
        try:
            ...
            if cached_data:
                ...
                return ETFBasicInfo(**cached_data)
    ...
    def get_etf_latest_data(self, etf_code: str) -> Optional[ETFPriceData]:
        ...
        try:
            ...
            # 保存到缓存
            self.cache_service.set_daily_cache(etf_code, latest_trade_date, latest_data.__dict__)
            ...
            price_data = ETFPriceData(
                ts_code=row.get('ts_code', etf_code),
                trade_date=row.get('trade_date', latest_trade_date),
                open=float(row.get('open', 0)) if row.get('open') else 0.0,
                high=float(row.get('high', 0)) if row.get('high') else 0.0,
                low=float(row.get('low', 0)) if row.get('low') else 0.0,
                close=float(row.get('close', 0)) if row.get('close') else 0.0,
                pre_close=float(row.get('pre_close', 0)) if row.get('pre_close') else 0.0,
                change=float(row.get('change', 0)) if row.get('change') else 0.0,
                pct_chg=float(row.get('pct_chg', 0)) if row.get('pct_chg') else 0.0,
                vol=float(row.get('vol', 0)) if row.get('vol') else 0.0,
                amount=float(row.get('amount', 0)) if row.get('amount') else 0.0
            )
            
            # 保存到缓存
            self.cache_service.set_daily_cache(etf_code, price_data.__dict__, latest_trade_date)
        ...
        try:
            ...
            for _, row in filtered_df.iterrows():
                basic_info = ETFBasicInfo(
                    ...
                    issue_amount=float(row.get('issue_amount', 0)) if row.get('issue_amount') else 0.0,
                    m_fee=float(row.get('m_fee', 0)) if row.get('m_fee') else 0.0,
                    c_fee=float(row.get('c_fee', 0)) if row.get('c_fee') else 0.0,
                    duration_year=float(row.get('duration_year', 0)) if row.get('duration_year') else 0.0,
                    p_value=float(row.get('p_value', 0)) if row.get('p_value') else 0.0,
                    min_amount=float(row.get('min_amount', 0)) if row.get('min_amount') else 0.0
                )
    ...
    def get_etf_summary(self, etf_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                'basic_info': basic_info.__dict__ if basic_info else None,
            ...
            if latest_data:
                summary['formatted_data'] = {
                    'current_price': format_currency(latest_data.close),
                    'change': format_currency(latest_data.change),
                    'change_percent': format_percentage(latest_data.pct_chg),
                    'volume': f"{latest_data.vol:,.0f}",
